...
void checkInput(struct tb_event *restrict ev, Color *restrict color, const wchar_t **restrict c,
  ...
  switch (ev->ch) {
    ...
    case 'S' : *sY = (*sY < tb_height() - 2) ? *sY + (++draw) : *sY; break; // clang-format off
    case 'D' : *sX = (*sX < tb_width()  - 1) ? *sX + (++draw) : *sX; break;
    // clang-format on
    // vi style movement and draw
    case 'K' : *sY = (*sY > 0) ? *sY - (++draw) : 0; break;                 // up
    case 'H' : *sX = (*sX > 0) ? *sX - (++draw) : 0; break;                 // left
    case 'J' : *sY = (*sY < tb_height() - 2) ? *sY + (++draw) : *sY; break; // clang-format off
    case 'L' : *sX = (*sX < tb_width()  - 1) ? *sX + (++draw) : *sX; break;
    // clang-format on
    // character select
...
void drawPalette(const wchar_t *restrict arr, const uint8_t len, const wchar_t *restrict c) {
  ...
  // TODO: constrain mouse draw area
